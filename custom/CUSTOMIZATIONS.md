...
| `lib/api/api.go` (9) | **Two lines**, registering `GET /rest/folder/claims` and `POST /rest/folder/claim`. The handlers are in a new file, `lib/api/api_claims.go`: "I'm working on this file", kept as one file per device in each folder's `.desuq-claims/`, so the claims reach exactly the people the folder is shared with and can never conflict among themselves. Authorship is taken from the index's `ModifiedBy`, never from the file name. Handing a mark to somebody else (`lib/api/api_handoff.go`) is a `handTo` field on the same `POST` and costs no further upstream line. | **Low.** Same append-only table; "keep both sides". |
| `lib/api/api.go` (10) | **One line**, registering `GET /rest/db/peerheldback`. The handler is in a new file, `lib/api/api_peerheldback.go`, and reaches the model through an optional interface satisfied by a new file inside `lib/model` (below), so `model.Model` and its generated mocks are untouched. Completion reads 100% for a peer who has taken one file of six, because an ignored file is not needed; this is the count from the peer's own index that says otherwise. | **Low.** Same table; "keep both sides". |
| `lib/api/api.go` (11) | **One line**, registering `GET /rest/db/whohas`, and one comment edited on the claim route to mention its new `auto` field. The handler is in a new file, `lib/api/api_whohas.go`: per person, whether their copy has this computer's version of a file -- including the people who are not connected, which upstream's `availability` list cannot say (it lists only connected devices, so somebody switched off reads the same as somebody without the file). Behind the Explorer "Who has this?" entry. | **Low.** Same append-only table; "keep both sides". |
| `lib/api/api.go` (12) | **Three lines**, registering `GET /rest/db/delivery`, `GET /rest/db/hub` and `POST /rest/db/hub/connect`. The handlers are in new files, `lib/api/api_delivery.go` (which of this computer's changes each peer does not have yet, from their remote need, offline peers included) and `lib/api/api_hub.go` (who in a folder only reaches whom through one computer, and adding that person directly). | **Low.** Same table; "keep both sides". |
| `lib/model/model.go` | **One line**, the first statement of `ClusterConfig` after `deviceID` is read: `m.desuqRememberClusterConfig(deviceID, cm)`. The method is in a new file, `lib/model/desuq_hub.go`, and keeps the device list each peer announces per folder -- which upstream reads for introductions and then drops -- so `/rest/db/hub` can say that two people only sync through this computer. Memory only, replaced on each primary cluster config. | **Low.** One additive line at the top of a long function. If upstream restructures `ClusterConfig`, re-add the call anywhere before the `Secondary` early return is fine too -- the method ignores secondary configs itself. If upstream renames `protocol.ClusterConfig` or its `Folders[].Devices`, it is a compile error in `desuq_hub.go`. |
| `lib/model/` (new files only) | **No upstream file is edited** by these (the one-line hook above is `desuq_hub.go`'s). `desuq_peerheldback.go` and `desuq_claimdelivery.go` are new files *inside* the package, for the same reason as `lib/versioner/desuq_open.go`: it needs the unexported database handle. It walks one peer's index for entries flagged `FlagLocalRemoteInvalid` and takes their sizes from the global entry, because the peer's own entry for an ignored file has its size blanked to zero. It also counts the peer's *receive-only local changes* separately (invalid too, but with size and block hash kept), which completion reads as "behind". `desuq_claimdelivery.go` compares a peer's index entry for one file against this device's, which is how a mark knows whether it has been delivered. | **None.** A new file cannot conflict; if upstream renames `m.sdb` or `AllLocalFiles`, it is a compile error. |
...
| `lib/api/api.go` (13) | **One line**, registering `POST /rest/folder/pin`. The handler is in a new file, `lib/api/api_pins.go`: pin one archived copy so the thirty-day cleanup never removes it. `/rest/folder/history` (already the fork's) reports which copies are pinned. | **Low.** Same append-only table; "keep both sides". |
| `lib/api/api.go` (14) | **Two lines**, registering `GET /rest/folder/notes` and `POST /rest/folder/note`. The handlers are in a new file, `lib/api/api_notes.go`: "why I changed this", one sentence per version of a file, written only by whoever saved that version, kept one file per device beside the marks in `.desuq-claims/`. `/rest/folder/history` (already the fork's) puts each note beside the archived copy it is about. | **Low.** Same append-only table; "keep both sides". |
| `lib/api/api.go` (15) | **One line**, registering `GET /rest/db/presence`. The handler is in a new file, `lib/api/api_presence.go`: per person sharing a folder, when this computer last saw them (the never-connected epoch filtered out) and the newest file whose current version they saved, read from the global index. | **Low.** Same append-only table; "keep both sides". |
| `lib/versioner/util.go` | **Two guards** for pinned versions (`desuq_pins.go`). In `cleanVersions`, the removal list is passed through `withoutPinned` -- the one place both the simple and the staggered versioner remove from, on every archive and every periodic clean. In `restoreFile`, a pinned copy is *copied* back with `osutil.Copy` instead of moved with `RenameOrCopy`, because a copy that leaves the archive loses its pin: the next save re-archives it under a new, unpinned timestamp. | **Medium.** Both are edits inside upstream functions rather than beside them. A conflict resolves by re-applying the filter to whatever `cleanVersions` removes, and the copy-not-move branch to whatever `restoreFile` becomes. **If upstream adds a new removal path, it must ask `isPinned` too**, and nothing will say so at compile time -- `desuq_pins_test.go` (run by `run-tests.ps1`) is what fails. |
| `lib/versioner/trashcan.go` | **One condition** in `Clean`: `&& !isPinned(...)` on the age check, so a pinned trashcan copy is kept. The trashcan has its own sweep and does not go through `cleanVersions`. | **Low.** One conjunct on one line. |
| `lib/versioner/` (new files) | `desuq_open.go` is a new file *inside* the package, which is how it can use the unexported `versionerFsFromFolderCfg` rather than reimplementing the archive path layout (a configured versions directory, a tilde in it, a relative one, a non-basic filesystem type). Getting a copy of that wrong fails silently — it looks like "no versions here" rather than an error. `desuq_pins.go` is the pin store, a JSON file beside this device's database keyed by archive directory, and the `isPinned` the two rows above call. Pins are this computer's only: the archive is not synced, and each device stamps its own copies. | **None.** A new file cannot conflict; the risk is that upstream renames the helper it calls, which is a compile error, not a silent one. |
...
| `lib/api/api_preview.go` | Thumbnails, decoded and box-sampled server-side. PNG/JPEG/GIF, and `.blend` via `desuq_blendthumb.go`; nothing else, which also keeps it from being a "read any file" route |
| `lib/api/api_claims.go` | `GET /rest/folder/claims`, `POST /rest/folder/claim`: who is working on which file. One `.desuq-claims/<device ID>.json` per device per folder, written atomically under Syncthing's temp prefix, removed when empty, hidden. A claims file is believed only when the index says its device last wrote it -- our own included |
| `lib/api/api_claims_test.go` | Path cleaning, claim and release, the per-device cap, a file that names somebody else inside, and History leaving claims out |
| `lib/api/api_peerheldback.go` | `GET /rest/db/peerheldback`: files and bytes a peer is deliberately not keeping |
| `lib/model/desuq_peerheldback.go` | The count behind it, from the peer's own index -- and, apart, what they changed in a copy that only receives |
| `lib/model/desuq_claimdelivery.go` | Whether a peer's index holds this device's version of one file. Behind "not seen by Kai yet" on a mark, and behind `whohas` |
| `lib/api/api_whohas.go` | `GET /rest/db/whohas`: who has this computer's version of a file, offline people included |
| `lib/api/desuq_blendthumb.go` | Reads the preview picture Blender embeds in every `.blend`: both header layouts (up to 4.x, and 5.x's `BLENDER17-01v0500` with 32-byte block headers), gzip, and zstd -- through `debug/elf`, because the standard library's zstd decoder is internal and a root `go.mod` dependency is the thing the fork avoids |
| `lib/api/desuq_blendthumb_test.go` | Synthetic files in every layout, compressed with gzip and with hand-built raw zstd frames; the refusals; and an opt-in run against real files (`DESUQ_BLEND_FILES`) |
| `lib/api/api_delivery.go` | `GET /rest/db/delivery`: per peer, which of this computer's changes they do not have yet -- counts, names, newest -- and everything else they are missing |
| `lib/api/api_delivery_test.go` | The tally: yours against theirs, deletions counted, directories and `.desuq-claims` not |
| `lib/api/api_hub.go` | `GET /rest/db/hub` and `POST /rest/db/hub/connect`: pairs who only sync through this computer, people reached only through a peer, and adding one of those directly |
| `lib/api/api_hub_test.go` | The middle, the edge, a full mesh, one side only, and nobody heard from |
| `lib/model/desuq_hub.go` | Keeps each peer's announced device list per folder. The one-line hook in `ClusterConfig` is its only upstream cost |
| `custom/tray/delivery.go` | "cabin.blend is now on Kai's computer" after a delivery that waited, and the undelivered half of the pause warning |
| `custom/tray/delivery_test.go` | Quiet for quick deliveries, the whole delivery named rather than the last minute of it, vanished peers dropped |
| `custom/tray/autoclaim.go` | Marks a `.blend` on save, and takes the mark off after four quiet hours. Reads saves from the disk feed; never inspects Blender's process |
| `custom/tray/reconnect.go` | "Back in touch with Kai": what a peer changed, and what you both changed, when two computers meet again |
| `custom/tray/resurrect.go` | "texture2.png came back": remembers local deletions on disk, and says when a later edit elsewhere restored one |
| `custom/tray/explorer.go` | The `-history` and `-who` one-shots behind the `.blend` right-click menu |
...
| `custom/tray/claims.go` | "Yuki is working on cabin.blend": the toast when somebody marks a file, the louder one when a file somebody else marked changes here, the `-claim` Send To one-shot, and the ignore exception that lets claims through a picker hold-back |
| `custom/tray/claims_windows.go`, `claims_other.go` | Hiding the claims directory where claims arrive; attributes do not sync |
| `custom/tray/claims_test.go` | Announce once, collisions from the OS-separator disk feed, the exception above the picker block and never twice, and Send To paths to folders (nested wins, case-blind) |
...
and `trashcan.go`, and 14 of those insertions. They are the first fork edits
to upstream's *cleanup* code, which deletes data, so they are the ones where a
merge that silently drops the guard costs something: a pinned copy would be
